...
export const createPost = (formData) => async (dispatch) => {
  ...
  try {
    const { data: post } = await axiosClient.post("/createPost", formData);
    console.log(post);
...
export const deletePost = (id) => async (dispatch) => {
  try {
    await axiosClient.delete(`/deletePost/${id}`);
...
export const likePost = (id) => async (dispatch) => {
  try {
    const { data: updatedPost } = await axiosClient.patch(`/likePost/${id}`);
    dispatch({ type: actionType.UPDATE_SUCCESS, updatedPost });
  } catch (error) {
    console.log(error);
  }
